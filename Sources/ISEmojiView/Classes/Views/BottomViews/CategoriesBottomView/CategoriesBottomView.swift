...
private let kMinCellSize = CGFloat(35)
...
final internal class CategoriesBottomView: UIView {
    ...
    private static let cellReuseIdentifier = "CategoryCell"
    ...
    @IBOutlet weak var collectionViewLeadingLayoutConstraint: NSLayoutConstraint!
    @IBOutlet weak var collectionViewTrailingLayoutConstraint: NSLayoutConstraint!
    
    @IBOutlet private weak var changeKeyboardButton: UIButton!
    @IBOutlet private weak var deleteButton: UIButton! {
        didSet {
            let normalImage = UIImage(named: "ic_delete_btn_icon", in: Bundle.podBundle,compatibleWith: nil)
            deleteButton.setImage(normalImage, for: .normal)
        }
    }

    @IBOutlet private weak var collectionView: UICollectionView! {
        didSet {
            collectionView.register(CategoryCell.self, forCellWithReuseIdentifier: Self.cellReuseIdentifier)
        }
    }
    ...
    static internal func loadFromNib(with categories: [Category], needToShowAbcButton: Bool, needToShowDeleteButton: Bool) -> CategoriesBottomView {
        ...
        bottomView.updateCollectionViewLayoutConstraints()
...
extension CategoriesBottomView: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: Self.cellReuseIdentifier, for: indexPath) as! CategoryCell
...
extension CategoriesBottomView {
    ...
    private func updateCollectionViewLayoutConstraints() {
        collectionViewLeadingLayoutConstraint.constant = changeKeyboardButton.isHidden ? (-1 * changeKeyboardButton.intrinsicContentSize.width + 12.0) : 0
        collectionViewTrailingLayoutConstraint.constant = deleteButton.isHidden ? (-1 * deleteButton.intrinsicContentSize.width + 12.0) : 0
    }
